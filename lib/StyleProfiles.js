 ...
 *   - foldMarginDelta: POSITIVE = folds more easily (tight), NEGATIVE = continues wider (loose)
 *     TAG is TIGHT (+3) → folds marginal spots, which PRESERVES chips for strong spots
 *     LAG is LOOSE (-2) → continues wider, compensated by aggressive raising
 *   - raiseMarginDelta: NEGATIVE = raises with less equity edge (aggressive)
 *     TAG and LAG both have -5, meaning they raise when equity exceeds pot odds by only 10% (vs Reg's 15%)
 *   - aggressionMult: Controls minBetEquity threshold and bet/raise confidence
 *     Higher values → bets with lower equity (leveraging fold equity)
 *
...
export const STYLE_PROFILES = {
  ...
  tag: {
    ...
    vpipTarget: 0.25,
    pfrRatio: 0.82,           // Raises 82% of hands played (high PFR ratio = aggressive entry)
    ...
    equityAdjustments: {
      foldMarginDelta: +3,    // TIGHT: folds marginal spots more readily (discipline)
      raiseMarginDelta: 0,    // AGGRESSIVE via preflop PFR and fold discipline, not thinner raises
      bluffFrequency: 0.12,   // Balanced bluffing (12%)
      aggressionMult: 1.20,   // 20% more aggressive
    },
  ...
  lag: {
    ...
    vpipTarget: 0.35,
    pfrRatio: 0.70,           // Raises 70% of hands played (aggressive entry, even loose)
    ...
    equityAdjustments: {
      foldMarginDelta: +1,    // Slightly tighter than Reg postflop (skilled LAG folds bad spots)
      raiseMarginDelta: -1,   // Slightly wider raises (VALUE_RAISE=14)
      bluffFrequency: 0.15,   // Frequent bluffs (15%)
      aggressionMult: 1.30,   // 30% more aggressive
    },

    sizingMult: 1.10,         // Slightly bigger bets for pressure
  },
