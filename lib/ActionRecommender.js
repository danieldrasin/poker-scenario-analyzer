...
function makeDecision(params) {
  ...
  if (!facingBet) {
    // Minimum equity needed to bet for value (scales with player count)
    // Heads-up: 50%, 3-way: 55%, 6-way: 65%
    const minBetEquity = 50 + (playersInHand - 2) * 3.5;

    // Strong made hand: bet for value (need decent hand type AND enough equity)
    if (adjustedEquity >= minBetEquity && handType >= 3) {
      const betType = determineBetType(adjustedEquity, handType, outs, isNuts);
      return {
        action: 'bet',
        confidence: Math.min(0.95, (0.6 + (adjustedEquity - minBetEquity) / 50) * aggressionMult),
        betType,
        reason: 'value_bet',
        heroStyle
      };
    ...
    // But NOT in multiway pots with non-nut hands — protection bets get called by better
    if (vulnerability > 0.4 && handType >= 3 && spr > 2 && (isNuts || playersInHand <= 3)) {
      return {
        action: 'bet',
